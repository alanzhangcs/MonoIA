...
def decode_detections(dets, info, calibs, cls_mean_size, threshold):
    ...
    for i in range(dets.shape[0]):  # batch
        ...
        for j in range(dets.shape[1]):  # max_dets
            ...
            if "trans_inv" in info.keys():
                trans_inv = info["trans_inv"][i]
                bbox[:2] = affine_transform(bbox[:2], trans_inv)
                bbox[2:] = affine_transform(bbox[2:], trans_inv)
                center2d = affine_transform([x, y], trans_inv)
                x, y = center2d[0], center2d[1]
                center3d = affine_transform([x3d, y3d], trans_inv)
                x3d, y3d = center3d[0], center3d[1]
